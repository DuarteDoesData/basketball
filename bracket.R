# 3/12/2020
# it would be helpful to have a master table that renames schools from different data sources
#     it only makes sense to build that once the 64 schools are picked


# try with last year's data first to see if predictions work
# what if this could "generate" all my picks with certain stats,
#     and then I just tweak it a little here and there

# links from other people
# https://www.sapanalytics.cloud/resources-march-madness/
# kenpom
# kaggle : https://www.kaggle.com/c/google-cloud-ncaa-march-madness-2020-division-1-mens-tournament/overview/timeline
# data source on team stats
...
import <- read_excel('/Users/horvasab/Desktop/bracket/sports_reference.xlsx', 
                         sheet = '2019') %>%
  ...
  mutate(school = tolower(stats_2019$School_trim)) %>%
  mutate(`3pg` = `3P`/Games) %>%
...
games_2020 <- merge(import3, lookup) 
...
compare <- matchup('duke', 'air force')
compare
...
games <- function(team1, team2){
  matchups <- games_2020 %>%
    ...
    select(Date, school, Final, opponent_final, opponent_school)
  matchups
}
winner <- games('clemson', 'duke')
